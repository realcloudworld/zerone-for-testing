...
Route::route('/api/extra/output/barcode/(.+)', function ($barcode) {
    ...
    $db = $database->open();
    $sth = $db->prepare("SELECT * FROM products WHERE barcode =:barcode AND is_active = 1");
    $sth->execute([
        'barcode' => $barcode,
...
Route::route('/api/extra/output/name/(.+)', function ($ar_name) {
    ...
    $database = new Connection();
    $db = $database->open();
    $sth = $db->prepare("SELECT * FROM products WHERE ar_name like  :ar_name AND is_active = 1");
...
Route::route('/api/extra/stocktake/barcode/(.+)/inventory_id/(.+)', function ($barcode, $inventory_id) {

    $database = new Connection();
    $db = $database->open();
    $sth = $db->prepare("SELECT * FROM products WHERE barcode =:barcode AND inventory_id = :inventory_id AND is_active = 1");
    $sth->execute([
        'barcode' => $barcode,
        'inventory_id' => $inventory_id
    ]);
...
Route::route('/api/extra/stocktake/barcode/(.+)', function ($barcode) {
...
Route::route('/api/extra/product/exists/ar_name/(.+)', function ($ar_name) {
    ...
    $database = new Connection();
    $db = $database->open();
    $sth = $db->prepare("SELECT * FROM products WHERE ar_name =:ar_name AND is_active = 1");
...
Route::route('/api/extra/product/exists/en_name/(.+)', function ($en_name) {
    ...
    $database = new Connection();
    $db = $database->open();
    $sth = $db->prepare("SELECT * FROM products WHERE en_name =:en_name AND is_active = 1");
...
Route::execute(urldecode(parse_url($_SERVER['REQUEST_URI'])['path']));
