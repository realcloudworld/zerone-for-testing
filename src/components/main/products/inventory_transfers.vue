      ...
      <v-card-text>
        ...
        <v-row>
          <v-data-table
            ...
            :items="inventory_transfers.stocktake_details"
            class="elevation-1"
...
export default {
  ...
  methods: {
    ...
    deleteItem(item) {
      this.inventory_transfers.stocktake_details.splice(
        this.inventory_transfers.stocktake_details.indexOf(item),
        1
      );
    },
    ...
    product_unit_change(item) {
      ...
      item.current_quantity = parseInt(
        item.details[0].quantity_in_minor_unit / unit.pivot.contains
      );
    ...
    submit() {
      if (this.is_new_stock)
        Inventory_transfers.store(this.inventory_transfers);
      else Inventory_transfers.update(this.inventory_transfers);
    },
    showThisProduct(selected_product) {
      if (
        this.inventory_transfers.stocktake_details.findIndex(
