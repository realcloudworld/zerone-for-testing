...
export default {
  ...
  methods: {
    getBills() {
      let bill_document_type_id = 1;
      if (this.route == "supplier" && this.receipt.payment_type_id == 1)
        bill_document_type_id = 1;
      else if (this.route == "supplier" && this.receipt.payment_type_id == 2)
        bill_document_type_id = 3;
      else if (this.route == "customer" && this.receipt.payment_type_id == 1)
        bill_document_type_id = 2;
      else if (this.route == "customer" && this.receipt.payment_type_id == 2)
    ...
    createPage(to, status) {
      ...
      Receipt.create({ document_type_id: this.receipt.document_type_id }).then(
        (response) => {
          this.people = response.data.people.filter(
            (elem) => elem.name != "افتراضي"
          );
          this.from_accounts = response.data.accounts.accounts;
          this.receipt.reference = response.data.receipt.reference;
        }
      );
    },
  },
